...
for entry in rdfield write-lines append-lines gc print read-lines file-existszzQ run_cmd run_capture run-argv run-capture-argv read type-of split argv argv0 getenv setenv exit make-dir delete-file copy-file; do
  printf '@set "RTENTRY=%s" & call _rt.cmd %%*\r\n' "$entry" > "$out/$entry.cmd"
done
echo "runtime: _rt.cmd (shared) + 22 entry stubs (rdfield/write-lines/.../copy-file)"
