#!/bin/sh
# Builds portsh-kernel.cmd and runs every fixture through it AS A SH SCRIPT (dash) —
...
for prog in "$here"/lisp/*.lisp; do
  ...
  got=$($runner "$root/portsh-kernel.cmd" "$prog")
  if [ "$got" = "$exp" ]; then
...
printf '\npass=%d fail=%d (woven portsh-kernel.cmd as %s)\n' "$pass" "$fail" "$runner"
[ "$fail" -eq 0 ]
