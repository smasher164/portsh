...
(define aref (lambda (r) (cond ((eq? (car r) (quote lit)) (cdr r)) ((eq? (car r) (quote raw)) (cdr r)) (t (str "!" (cdr r) ":~2!")))))
(define iref (lambda (r) (cond ((eq? (car r) (quote lit)) (cdr r)) ((eq? (car r) (quote raw)) (str "!" (cdr r) "!")) (t (str "!" (cdr r) ":~2!")))))
(define vref (lambda (r) (cond ((eq? (car r) (quote lit)) (str "I:" (cdr r))) ((eq? (car r) (quote raw)) (str "I:!" (cdr r) "!")) (t (str "!" (cdr r) "!")))))
...
(define cexpr (lambda (f pmap k)
  (cond
    ...
    ((eq? (car f) (quote car)) (ccell f "CAR_" pmap k))
    ((eq? (car f) (quote cdr)) (ccell f "CDR_" pmap k))
    (t (let ((ar (cargs (cdr f) pmap k)))
         (let ((tmp (str "zt" (number->string (caddr ar)))))
           (list (append (car ar) (list (str "call compiled.cmd " (symbol->string (car f)) (cadr ar)) (str "set " tmp "=!R!")))
                 (cons (quote val) tmp) (+ (caddr ar) 1))))))))
;; car/cdr: strip P: from the (val) operand to an index, then read CAR_/CDR_<idx>.
(define ccell (lambda (f field pmap k)
  (let ((rx (cexpr (cadr f) pmap k)))
...
;; cargs: call args rendered as TAGGED values -> (list lines " v v ..." k)
...
(define ctail (lambda (f nm lbl ps pmap k)
  (cond
    ...
    (t (let ((r (cexpr f pmap k))) (cons (append (car r) (list (str "set R=" (vref (cadr r))) "goto :eof")) (caddr r)))))))
